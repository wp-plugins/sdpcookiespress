...
jQuery(document).ready(function () {
    ...
    jQuery('form#sdpCookiesForm').submit(function(event) {
        if(APIkey == false){
            event.preventDefault();
            var posting = jQuery.post( "http://welcome.smartdataprotection.eu/es/services/validateapi/"+jQuery('input[name="apiKey"]').val() );
...
function validateAPI(api) {
    var posting = jQuery.post( "http://welcome.smartdataprotection.eu/es/services/validateapi/"+api );
    jQuery('span.errorAPI').hide();
    posting.done(function( data ) {
        if(typeof data.license != 'undefined' && data.license > 0){
